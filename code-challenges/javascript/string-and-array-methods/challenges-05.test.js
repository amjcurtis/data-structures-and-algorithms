...
const howMuchPencil = (str) => {
  ...
  for (let i = 0; i <= str.length; i++) {
    let slicedElement = str.slice(i, str.length); // Second param is optional
...
const wordsToCharList = (arr) => arr.split(''); // (Implicit return)
...
const listFoods = (recipe) => {
  ...
  // Solution code (part of it) that we walked thru in class
  recipe.ingredients.forEach((ingredient) = {
    let withoutAmt = ingredient.slice(ingredient.indexOf(' ') +1)
    let withoutUnits = withoutAmt.slice(withoutAmt.indexOf(' ') +1)
